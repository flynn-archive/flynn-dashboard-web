...
var Login = FlynnDashboard.Views.Models.Login = Marbles.Utils.createClass({
	...
	mixins: [Marbles.State, {
		ctor: {
			validationRequiredKeypaths: ["email", "passphrase"],

			validation: {
				"email": function (email) {
					return new Promise(function (resolve, reject) {
						if (email.match(/^[^@\s]+@[^\s]{3,}$/)) {
							resolve();
						} else {
				...
				"passphrase": function (passphrase) {
					return new Promise(function (resolve, reject) {
						if (passphrase.length >= 6) {
							...
							reject("Passphrase must be at least six characters long");
						}
					});
				}
			}
	...
	performLogin: function () {
		...
		return FlynnDashboard.client.login(attrs.email, attrs.passphrase).then(function () {
			this.__persisting = false;
			this.setState({});
